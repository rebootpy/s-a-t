<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<!---FAVICON-->
	<link rel="icon" type="image/x-icon" href="https://verificacionsat.ml/assets/images/sat.png">
	<!--STYLES-->
	<link rel="stylesheet" href="assets/validacion.css">
	<link rel="stylesheet" href="assets/styles.css">
	<!--FONTAWESOME-->
	<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.4.1/css/all.css">
	<title>S-A-T</title>
</head>
<body ondragstart="return false;" ondrop="return false;">
	<div class="fixed">
		<nav class="navbar">
            <div class="navbar-header">
                <a href="#" class="navbar-brand"><img src="https://verificacfdi.facturaelectronica.sat.gob.mx/Content/images/logoheader.svg" alt="Gobierno de México"></a>
                <button class="navbar-toggler"><span></span></button>
            </div>
            <div class="navbar-collapse">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a href="error.php" class="nav-link"><i class="fas fa-concierge-bell"></i> Trámites</a>
                    </li>
                    <li class="nav-item">
                        <a href="error.php" class="nav-link"><i class="fas fa-address-card"></i> Nosotros</a>
                    </li>
                </ul>
            </div>
        </nav>
	</div>
	<!--WELCOME-->
	<!--<div class="transport_web">
		<div class="info_web">
			<ul>
				<li><a href="https://www.gob.mx"><i class="fas fa-home"></i></a></li> /
				<li><a href="">Inicio</a></li>
			</ul>
		</div>
	</div>-->
	<!--END-->
	<section class="saldo_rezagado">
		<div class="label">
			<p>Tu saldo rezagado esta en proceso</p>
			<h1>Necesita aprobación de un ejecutivo, esto puede demorar de 7-12 dias.</h1>
			<a href="index.php">Volver al inicio</a>
		</div>
	</section>
</body>
</html>